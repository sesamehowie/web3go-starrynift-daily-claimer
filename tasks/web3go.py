...
class Web3Go(Client):
    ...
    async def claim_leaves(self, browser, password):
        context = browser.contexts[0]
        page = await self.setup_context(context)
        await self.metamask_auth(page, password=password)
        await page.goto('https://reiki.web3go.xyz/aiweb/home')
        await page.wait_for_load_state()
        await page.reload()
        await page.wait_for_load_state()
        await expect(page.locator('//html/body/div[1]/div/div[1]/div/div/div[1]/div[2]/img[2]')).to_be_visible(timeout=10000)
        await page.locator('//html/body/div[1]/div/div[1]/div/div/div[1]/div[2]/img[2]').click(force=True)
        await expect(page.get_by_text('Log in with Metamask')).to_be_visible()
        async with context.expect_page() as new_page_info:
            await page.get_by_text('Log in with Metamask').click(force=True)
        meta_noti = await new_page_info.value
        await meta_noti.wait_for_load_state()
        await expect(meta_noti.locator('//html/body/div[1]/div/div/div/div[5]/footer/button[2]')).to_be_visible()
        await meta_noti.locator('//html/body/div[1]/div/div/div/div[5]/footer/button[2]').click(force=True)
        await page.wait_for_load_state()
        await page.reload()
        await page.wait_for_load_state()
        await expect(page.locator('//html/body/div[1]/div/div[1]/div/div/div[1]/div[2]/img[2]')).to_be_visible()
        await page.locator('//html/body/div[1]/div/div[1]/div/div/div[1]/div[2]/img[2]').click(force=True)
        await page.wait_for_load_state()
        url = page.url
        if url == 'https://reiki.web3go.xyz/mint':
            try:
                await expect(
                    page.locator('//*[@id="root"]/div/header/nav/div/ul/li[2]/img')).to_be_visible()
                await page.locator('//*[@id="root"]/div/header/nav/div/ul/li[2]/img').click(force=True)
                await page.wait_for_url('https://reiki.web3go.xyz/taskboard')
                curr_date = strftime('%m.%d')
                await expect(page.get_by_text(curr_date)).to_be_visible()
                await expect(page.locator(
                    '//html/body/div[1]/div/main/div[2]/div[2]/div/div[2]/div[2]/div/div[3]/div[3]/a')).to_be_visible()
                await page.locator(
                    '//html/body/div[1]/div/main/div[2]/div[2]/div/div[2]/div[2]/div/div[3]/div[3]/a').click(
                    force=True)
            except Exception as e:
                logger.error(e)
        elif url == 'https://reiki.web3go.xyz/taskboard':
            try:
                curr_date = strftime('%m.%d')
                await expect(page.get_by_text(curr_date)).to_be_visible()
                await expect(page.locator(
        ...
        await page.wait_for_load_state()
        url = page.url
        if url == 'https://reiki.web3go.xyz/taskboard' or url == 'https://reiki.web3go.xyz/mint':
            await page.wait_for_load_state()
            await expect(page.locator('//*[@id="root"]/div/header/nav/div/ul/li[3]/div/div/div')).to_be_visible()
            if await page.locator('//*[@id="root"]/div/header/nav/div/ul/li[3]/div/div/div').is_enabled():
                await page.locator('//*[@id="root"]/div/header/nav/div/ul/li[3]/div/div/div').click()
                await page.wait_for_load_state()
                await page.wait_for_selector('//html/body/div[3]/span/div/div/div[2]')
                await expect(page.locator(
                    '//html/body/div[3]/span/div/div/div[2]')).to_be_visible()
                await page.locator('//html/body/div[3]/span/div/div/div[2]').click()
                await page.wait_for_load_state()
                        ...
                        async with aiohttp.ClientSession() as session:
                            ...
                            if port and ws_endpoint:
                                async with async_playwright() as p:
                                    ...
                                    await self.claim_leaves(browser, password)
